"""
Seed data script for CeleCircle
...
def seed_data():
    ...
    try:
        ...
        existing_admin = db.query(User).filter(User.email == "admin@celecircle.com").first()
        if existing_admin:
            print("Database already has seed data. Skipping seed creation.")
            print("If you want to recreate, delete celecircle.db and run again.")
            return
        
        # Create admin user
        admin = User(
            email="admin@celecircle.com",
        ...
        print(f"   - Admin: admin@celecircle.com / admin123")
