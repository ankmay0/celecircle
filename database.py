...
def init_db():
    from models import (
        ...
        Dispute, SystemSetting, CelebrityNews, AuditLog, Payout
    )
    ...
    with engine.connect() as conn:
        ...
        conn.execute(text("CREATE INDEX IF NOT EXISTS ix_celebrity_news_published_at ON celebrity_news (published_at);"))
        conn.execute(text("CREATE INDEX IF NOT EXISTS ix_celebrity_news_is_approved ON celebrity_news (is_approved);"))
        conn.execute(text("CREATE UNIQUE INDEX IF NOT EXISTS uq_celebrity_news_article_url ON celebrity_news (article_url);"))
        conn.execute(text("ALTER TABLE celebrity_news ADD COLUMN IF NOT EXISTS region VARCHAR DEFAULT 'india';"))
        ...
        if existing == 0:
            conn.execute(text(
                ...
                "('cele_news_last_fetch_time', '', 'Last successful celebrity news fetch timestamp (ISO UTC)'),"
                "('auto_approve_news', 'true', 'Auto approve newly fetched celebrity news')"
            ))
        else:
            ...
            has_news_fetch = conn.execute(text("SELECT COUNT(*) FROM system_settings WHERE key='cele_news_last_fetch_time'")).scalar()
            if not has_news_fetch:
                conn.execute(text(
                    "INSERT INTO system_settings (key, value, description) VALUES "
                    "('cele_news_last_fetch_time', '', 'Last successful celebrity news fetch timestamp (ISO UTC)')"
                ))
            has_auto_approve = conn.execute(text("SELECT COUNT(*) FROM system_settings WHERE key='auto_approve_news'")).scalar()
