            ...
            <div class="header-container">
                <div class="header-left">
                    <a href="/feed" class="logo-header">
                        <img src="/static/logos/celevircle-logo.png" alt="celevircle logo" class="logo-brand-image">
                    </a>
                ...
                <nav class="header-nav app-main-nav">
                    ...
                    <button type="button" class="nav-item-header nav-theme-toggle" id="themeToggleBtn" onclick="toggleNavbarTheme()" title="Toggle dark/light theme">
                        <span class="nav-icon-header" id="themeToggleIcon">🌙</span>
                        <span class="nav-label" id="themeToggleLabel">Dark</span>
                    </button>
                </nav>
            </div>
        </header>
...
function attachMobileProfileNavigation() {
    const profileNav = document.getElementById('nav-me');
    if (!profileNav || profileNav.hasAttribute('data-mobile-profile-bound')) {
        return;
    }
    ...
    profileNav.setAttribute('data-mobile-profile-bound', 'true');
}

// Try to attach listeners after a short delay
setTimeout(attachNotificationListeners, 200);
setTimeout(attachMobileProfileNavigation, 220);
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(attachNotificationListeners, 100);
    setTimeout(attachMobileProfileNavigation, 120);
});
...
function createMeDropdown() {
    ...
    if (userRole === 'admin') {
        ...
        <div class="dropdown-section">
            ...
            <a href="/admin/news" class="dropdown-item">
                <span class="dropdown-icon">📰</span>
                <span>CeleNews</span>
            </a>
...
async function displaySearchResults(results) {
    ...
    // Defense-in-depth: never render admin users in global search results.
    results = (results || []).filter(r => String(r.role || '').toLowerCase() !== 'admin');
