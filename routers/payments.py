...
from services.common.event_bus import get_event_bus
import logging
import os
...
def _log_failure(event_name: str, reason: str, context: dict):
    payload = {"event": event_name, "reason": reason, **context}
    logger.error("payment_failure %s", payload)
...
def create_payment(
    ...
    try:
        ...
        _log_failure(
            "PaymentFailed",
            str(e),
            {"stage": "create_intent", "gig_id": payment_data.gig_id, "organizer_id": current_user.id},
        )
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Payment processing temporarily failed",
        )
    ...
    try:
        ...
        _log_failure(
            "PaymentFailed",
            str(e),
            {"stage": "persist_payment", "gig_id": payment_data.gig_id, "organizer_id": current_user.id},
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Could not persist payment record",
        )
...
def confirm_payment_endpoint(
    ...
    if not USE_MOCK_PAYMENTS:
        try:
            ...
            _log_failure(
                "PaymentFailed",
                str(e),
                {"stage": "confirm", "payment_id": payment.id, "organizer_id": current_user.id},
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        _log_failure(
            "PaymentFailed",
            str(e),
            {"stage": "persist_confirm", "payment_id": payment.id, "organizer_id": current_user.id},
        )
        raise HTTPException(status_code=500, detail="Could not save payment confirmation")
...
def release_payment_endpoint(
    ...
    if not USE_MOCK_PAYMENTS:
        try:
            ...
            _log_failure(
                "PaymentFailed",
                str(e),
                {"stage": "release", "payment_id": payment.id, "organizer_id": current_user.id},
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Payment release failed",
            )
    ...
    try:
        ...
        _log_failure(
            "PaymentFailed",
            str(e),
            {"stage": "persist_release", "payment_id": payment.id, "organizer_id": current_user.id},
        )
        raise HTTPException(status_code=500, detail="Could not persist payment release")
...
def refund_payment_endpoint(
    ...
    if not USE_MOCK_PAYMENTS:
        try:
            ...
            _log_failure(
                "RefundFailed",
                str(e),
                {"stage": "refund", "payment_id": payment.id, "organizer_id": current_user.id},
            )
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    try:
        ...
        _log_failure(
            "RefundFailed",
            str(e),
            {"stage": "persist_refund", "payment_id": payment.id, "organizer_id": current_user.id},
        )
        raise HTTPException(status_code=500, detail="Could not persist refund state")
