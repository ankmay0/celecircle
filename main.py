...
from services.news_service import fetch_celebrity_news
...
app = FastAPI(
    title="CeleCircle API",
    description="Entertainment Industry Marketplace API",
    version="1.0.0"
)


def _cele_news_scheduler_worker():
    # Run a periodic fetch loop in background.
    while True:
        db = SessionLocal()
        try:
            fetch_celebrity_news(db, force=False)
        except Exception:
            pass
        finally:
            db.close()
...
async def admin_news_page(request: Request):
    return HTMLResponse(content=render_template("admin-news.html"))
...
def health_check():
    ...
    return {"status": "healthy", "service": "CeleCircle API"}


@app.on_event("startup")
async def start_background_jobs():
    # Initial attempt with caching guard.
    db = SessionLocal()
    try:
        fetch_celebrity_news(db, force=False)
    except Exception:
    ...
    worker = threading.Thread(target=_cele_news_scheduler_worker, daemon=True)
    worker.start()
